...
class WikiDataStep1Manager(DataManager):
    class WikiDatasetStep1Filtered(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            labels_ent = torch.full(
                (self.max_num_entity * 2,), -1
            )  # Using -1 for padding

            # Sort the entities by the length of the entity name in descending order
            sorted_entities_by_len = sorted(
                entities.items(),
                key=lambda item: len(item[1]["pk_entity_name"]),
                reverse=True,
            )
            ...
            for i, (entity_key, entity_values) in enumerate(sorted_entities_by_len):
                ...
                if start_token_pos != -1:
                    ...
                    placeholder_tokens = [
                        "<extra_id_{}>".format(j) for j in range(len(entity_tokens))
                    ]
                    tokenized_text[start_token_pos : end_token_pos + 1] = (
                        placeholder_tokens
                    )
                else:
            ...
            filtered_spans = [
                span for span in entity_spans if span[0] != -1 and span[1] != -1
            ]
            ...
            labels_ent_tokens = [
                position_to_token(pos) for pos in labels_ent if pos != -1
            ]
            ...
            sorted_entities_by_len = [
                sorted_entities_by_len[span_info[2]] for span_info in sorted_spans
            ]
            sorted_entities_by_occur = {
                str(index): value
                for index, (_, value) in enumerate(sorted_entities_by_len)
            }

            # Extracting entity names and formatting them with [SEP] token
            entity_names = [
                entity["pk_entity_name"] for entity in sorted_entities_by_occur.values()
            ]
            ...
            labels_ent_name = self.tokenizer(
                real_labels_ent_name,
                max_length=self.max_num_entity * 5,
                padding="max_length",
                truncation=True,
            ...
            for i, (entity_key, entity_values) in enumerate(
                sorted_entities_by_occur.items()
            ):
                if (
                ...
                for pk in entity_values:
                    if pk != "pk_type":  # Exclude the type itself, already added
                ...
                if pk_counter > 1 and pk_counter < labels_pk.size(1):
            ...
            for i, (entity_key, entity_values) in enumerate(
                ...
                pk_counter = 0
                for pk in entity_values:
                    if pk != "pk_type":  # Skip the entity type itself
            ...
            return {
                ...
                "labels_ent_name": labels_ent_name,  # (max_num_Entity * 5, )
                "real_labels_ent_name": real_labels_ent_name,  # (max_num_Entity * 5, )
    ...
    def create_dataset(self, file_path, model_name, use_data, max_length, **kwargs):
    ...
    def create_data_loader(
        ...
        **kwargs,
        ...
        dataset, tokenizer, special_tokens_need_to_add, ent_type_tokens, pk_tokens = (
            self.create_dataset(file_path, model_name, use_data, max_length, **kwargs)
        )
